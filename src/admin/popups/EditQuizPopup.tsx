...
import { Answer, Course, Question, QuestionType, Quiz } from "../../utils/models";
import plusSign from "../../assets/plus-sign.png"
import xSign from "../../assets/x-sign.png"
import tickSign from "../../assets/tick-sign.png"
import { updateQuiz } from "../../services/adminService";
import { getCoursesWithoutQuiz } from "../../services/courseService";

interface EditQuizPopupProps {
    quiz: Quiz;
    questionTypes: QuestionType[];
    closeEditPopup: () => void;
}

export const EditQuizPopup = ({quiz, questionTypes, closeEditPopup}: EditQuizPopupProps) => {
    let [updatedQuiz, setUpdatedQuiz] = useState<Quiz>(quiz);
    let [courses, setCourses] = useState<Course[]>([]);
    
    useEffect(() => {
        loadCourses();
        setCourses(prevCourses => ([...prevCourses, quiz.course]));
    ...
    const saveQuiz = () => {
        const req = {
            title: (quiz.title !== updatedQuiz!.title) ? updatedQuiz!.title : null,
        ...
        updateQuiz(req, updatedQuiz.id).then(() => {
            closeEditPopup();
        });
    };

    const canSaveQuiz = () => {
        return updatedQuiz.title && updatedQuiz.course && updatedQuiz.questions.length > 0;
    };
            ...
            <div className="edit-course-popup-container">
                <div className="basic-info">
                    ...
                    <label>
                        ...
                        <select
                            name="courseId"
                            value={updatedQuiz.course ? updatedQuiz.course.id : -1}
                ...
                <div className="add-section">
                    ...
                    <button onClick={closeEditPopup} disabled={isAnyQuestionBeingEdited()}>CANCEL</button>
                </div>
            </div>
        }
    </>);
